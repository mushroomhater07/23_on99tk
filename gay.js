  ...
  var search = $("#searchon").val().trim();    
    if(txt == "PH viewer"){
      if(search.length != 0){
        ...
        window.location.href=`https://www.pornhub.com/gay/video/search?search=${search}`;
      }else{
      ...
      if(search.length != 0){
        search = search.replace(" ","+")
        window.location.href=`https://xhamster.com/search/${search}?orientations=gay`;
      }else{
        alert("are you fking kidding me?")
      }
...
function getdata(){
  $.get(" raw_profile.csv", (data)=>{objs = d3.csvParse(data);console.log(objs)
    starindex = Math.floor(Math.random() * objs.length)});
  $.get(" gay.txt", (data)=>{objs1 = d3.tsvParse(data); 
